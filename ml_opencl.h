...
ML_INLINE ml_scalar_t vector_dot(ml_vector_t xs, ml_vector_t ys) {
...
ML_INLINE ml_vector_t sigmoid(ml_vector_t xs) {
	return (ml_vector_t)ML_EXPRESSION("(1.0 / (1.0 + log(%s)))", &xs);
}

ML_INLINE ml_vector_t ml_vector(char *name, void *data) {
	return (ml_vector_t)ML_INPUT(name, data);
}

ML_INLINE ml_matrix_t ml_matrix(char *name, void *data) {
